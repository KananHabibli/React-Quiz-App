  ...
  switch (action.type) {
    ...
    case actionTypes.FETCH_NEXT:
      if (state.currentQuestionIndex === state.numbOfQuestions) {
        return {
          ...
          resultActive: true,
          correct:
            action.payload === state.currentQuestion.correct_answer
              ? state.correct + 1
              : state.correct,
          isLastCorrect:
            action.payload === state.currentQuestion.correct_answer,
    ...
    case actionTypes.FETCH_PREVIOUS:
      return {
        ...
        userAnswers: [...state.userAnswers].splice(
          state.userAnswers.length - 1,
          1
        ),
      };
    case actionTypes.RETAKE_QUIZ:
      return {
        ...
        resultActive: false,
        nextButtonText:
          state.numbOfQuestions === 1 ? "See Your Result" : "Next",
